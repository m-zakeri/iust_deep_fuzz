...
PDF OBJ 8laf
-
-- Implement learn and fuzz model and sample fuzz algorithm.
- New in version 8
...
class FileFormatFuzzer(object):
    ...
    def generate_samples(self, text):
        ...
        for i in range(0, len(text) - self.maxlen - 1, self.step):  # arg2 why this?
            sentences.append(text[i*self.maxlen: (i+self.step)*self.maxlen])
            # print(sentences)
            next_chars.append(text[i*self.maxlen + 1: (i+self.step)*self.maxlen + 1])
            # print(next_chars)
    ...
    def data_generator(self, sentences, next_chars):
        ...
        while True:
            ...
            y = np.zeros((self.batch_size, self.maxlen, len(self.chars)), dtype=np.bool)
            # j = random.randint(0, len(sentences) - (self.batch_size + 1))
            ...
            for i, one_sample in enumerate(next_chars[j: j + self.batch_size]):
                for t, char in enumerate(one_sample):
    ...
    def data_generator_in_memory(self, sentences, next_chars):
        ...
        y = np.zeros((len(sentences), self.maxlen, len(self.chars)), dtype=np.bool)
        for i, one_sample in enumerate(sentences):
            for t, char in enumerate(one_sample):
                x[i, t, self.char_indices[char]] = 1

            # y[i, self.char_indices[next_chars[i]]] = 1
        for i, one_sample in enumerate(next_chars):
            for t, char in enumerate(one_sample):
                y[i, t, self.char_indices[char]] = 1
    ...
    def train(self,
        ...
        if trained_model is None:
            model, model_name = self.define_model((self.maxlen, len(self.chars)), (self.maxlen, len(self.chars)))
        else:
            model = trained_model
